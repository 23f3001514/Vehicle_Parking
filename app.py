...
#     if spot.is_occupied:
#         return "Spot is already Booked!" , 403
    
#     spot.is_occupied = True
#     spot.booked_by = session['user.id']
...
#     if spot.booked_by != session['user.id']:
#         return "You can Canel Your Own Bookings!" , 403
...
# Task 1: Prevent Admin from creating a parking lot with a duplicate address.
# In your /admin/create_lot route, currently the admin can add a parking lot even if one with the same address already exists.

# 👉 Can you write a code snippet that checks if the address already exists in the ParkingLot table?
# If it does, show an error on the same page: "A lot with this address already exists."
...
#             ParkingLot.location.ilike(f'%{ search_query }%')
#         ).all()
...
#             for spot in lot.spots:
#                 if spot.is_occupied:
#                     spot.booked_user = User.query.get('user.id')
#                 else:
#                     None

#     return render_template('admin_dashboard.html' , lots=lots , search_query=search_query , username = session['admin_username'])
...
#     if spot.is_occupied:
#         return "This Spot is already Booked!" , 400
    
#     spot.is_occupied = True
#     spot.booked_by = User.query.get(session['user.id'])
#     db.session.commit()
...
#         if User.query.filter_by(username=username).first():
#             return render_template('uer.register.html' , error = "Username already Exists!")
        
#         new_user = User(username=username , email_id=email_id , password=password , address=address , pin_code=pin_code)
...
#         user = User(username = username,email_id = email_id, password = password, address = address, pin_code = pin_code)
#         db.session.add(user)
#         db.session.commit()
#         return redirect('/user/login')
#     return render_template('user_register.html')
